...
-   **Repeated game**:
    ...
    -   Discount factor $\delta = \frac{1}{1+i}$
...
>
> $$
> PV = \frac{FV}{(1+i)^t} = FV \cdot \delta^t
> $$
...
Where $\Pi_A^\text{cheat}$ = Present value of firm A when it cheats
...
>
> -   Because players know when the game will end, they have an **incentive to cheat in the final period**.
...
> -   This reasoning continues all the way back to the first round, so the **only subgame perfect equilibrium is the one-shot Nash equilibrium played in every period**.
